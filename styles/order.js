...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const tariffFromUrl = urlParams.get('tariff') || 'Smart';
    ...
    if (orderForm) {
        orderForm.addEventListener('submit', async function(event) {
            ...
            if (sendBtn) {
                ...
                if (btnText) btnText.innerText = 'Надсилається...';
            }
            ...
            try {
                ...
                alert('Зачекайте, сервер запускається (це займає до 1 хв). Спробуйте ще раз, якщо помилка повториться, зверніться до адміністратора.');
                if (sendBtn) {
